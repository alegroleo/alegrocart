...
class ControllerModuleExtraDeveloper extends Controller {
	var $error = array();  	// All References change to module_extra_ due to new module loader  
 	function __construct(&$locator){
		...
		$this->adminController = $this->template->set_controller('module_extra_developer');
	...
	function index(){
		...
		if (($this->request->isPost()) && ($this->validate())) {
			...
			if ($this->request->has('update_form', 'post')) {
				$this->response->redirect($this->url->ssl('module_extra_developer'));
			} else {
		...
		$this->session->set('last_module', 'module_extra_developer');
		$this->session->set('last_extension_id', $this->modelDeveloper->get_extension_id('module_extra_developer'));
	...
	function uninstall() {
		if ($this->user->hasPermission('modify', 'module_extra_developer')) {
			$this->modelDeveloper->delete_developer();
			if ($this->session->get('last_module') == 'module_extra_developer') {

...
class Model_Admin_Order extends Model {
	...
	public function update_status_history($order_id, $order_status_id){
	...
	public function insert_order_history(){
		$sql = "insert into order_history set order_id = '?', order_status_id = '?', date_added = now(), notify = '?', comment = '?'";
	...
	public function get_modified_order($new_reference){
		$result = $this->database->getRow("select order_id, date_added from `order` where reference = '" . $new_reference . "'");
		return $result;
	}

	public function get_products(){
		$results = $this->database->getRows("SELECT op.*, p.product_id FROM order_product op LEFT JOIN product p ON (op.product_id = p.product_id) WHERE order_id = '" . (int)$this->request->gethtml('order_id') . "' ORDER BY order_product_id");
		return $results;
	}
	...
	public function get_history(){
		$results = $this->database->getRows("select oh.date_added, os.name as status, oh.comment, oh.notify from order_history oh left join order_status os on oh.order_status_id = os.order_status_id where oh.order_id = '" . (int)$this->request->gethtml('order_id') . "' and os.language_id = '" . (int)$this->language->getId() . "' order by oh.date_added");
		return $results;
	}

	public function get_vendor($order_product_id){
		$result = $this->database->getRow("select vendor_id, vendor_name from order_product where order_product_id = '" . (int)$order_product_id . "'");
		return $result;
	}

	public function check_downloads($order_product_id){
	...
	public function get_page(){
		...
		if ($this->session->get('default_order_status')){
			$sql .= " AND o.order_status_id ='" . (int)$this->session->get('default_order_status') . "'";
		}

...
class ControllerShippingCanadaPost extends Controller{
	...
	function index() {  
		...
		if ($this->request->isPost() && $this->request->has('global_canadapost_status', 'post') && $this->validate()) {
			...
			if ($this->request->has('update_form', 'post')) {
				$this->response->redirect($this->url->ssl('shipping_canadapost'));
			} else {
		...
		$this->session->set('last_shipping', 'shipping_canadapost');

		$this->session->set('name_last_shipping', $this->language->get('heading_title'));
		$this->session->set('last_shipping', 'shipping_canadapost');
		$this->session->set('last_extension_id', $this->modelCANpost->get_extension_id('shipping_canadapost'));
	...
	function uninstall() {
		if ($this->user->hasPermission('modify', 'shipping_canadapost')) {
			$this->modelCANpost->delete_CANpost();
			if ($this->session->get('last_shipping') == 'shipping_canadapost') {
	...
	function tab() {
		if ($this->request->isPost()) {
			if ($this->request->has('activeTab', 'post')) {
				$this->session->set('shipping_canadapost_tab', $this->request->sanitize('activeTab', 'post'));
			}
		}
	}
}

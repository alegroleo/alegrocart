...
class ControllerModuleCatalogcurrency extends Controller {
	...
	function __construct(&$locator){
		...
		$this->adminController = $this->template->set_controller('module_catalog_currency');
	...
	function index() { 
		...
		if ($this->request->isPost() && $this->request->has('catalog_currency_status', 'post') && $this->validate()) {
			...
			if ($this->request->has('update_form', 'post')) {
				$this->response->redirect($this->url->ssl('module_catalog_currency'));
			} else {
		...
		$this->session->set('last_module', 'module_catalog_currency');
		$this->session->set('last_extension_id', $this->modelCurrency->get_extension_id('module_catalog_currency'));
	...
	function uninstall() {
		if ($this->user->hasPermission('modify', 'module_catalog_currency')) {
			$this->modelCurrency->delete_currency();
			if ($this->session->get('last_module') == 'module_catalog_currency') {

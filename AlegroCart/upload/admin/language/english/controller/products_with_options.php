<?php
// Heading
$_['heading_title']        = 'Products with Options';
$_['heading_form_title']   = 'Product with Options:';
$_['heading_description']  = 'You can edit your products with options here.';

// Text
$_['text_message']         = 'Success: You have updated products!';
$_['text_select']          = 'Select Product';
$_['text_linear']          = 'Linear: Length, Width, Height';
$_['text_area']            = 'Area: Length X Width';
$_['text_volume']          = 'Volume: Litre, Quart';
$_['text_no_dimensions']   = 'There are no dimensions available for <b> %s </b>';
$_['text_no_dim']          = 'None';
$_['text_no_image']        = 'No Image Selected';
$_['text_dimension_ship']  = 'If type is changed from settings default, you must enter a value appropriate for the new type. <br>Length, width, height are optional, used for shipping calculations where cubing is applied.';
$_['text_options']         = 'Options: ';
$_['text_quantity_options']= 'Add Quantities in Product Options';
$_['text_upc']            = 'UPC';
$_['text_ean']            = 'EAN-13/ISBN';

// Column
$_['column_product_option']= 'Product ID';
$_['column_name']          = 'Product : ';
$_['column_stock']         = 'Stock';
$_['column_image']         = 'Image';
$_['column_model_number']  = 'Model Number';
$_['column_action']        = 'Action';
$_['button_refresh']       = 'Choose New Product';

// Entry
$_['entry_select_product'] = 'Select Main Product with Options: ';
$_['entry_dimension_class']= 'Dimension Type:';
$_['entry_dimension']      = 'Default Dimension:';
$_['entry_length']         = 'Length:';
$_['entry_width']          = 'Width:';
$_['entry_height']         = 'Height:';
$_['entry_area']           = 'Area:';
$_['entry_volume']         = 'Volume:';
$_['entry_quantity']       = 'In Stock Quantity:';
$_['entry_model_number']   = 'Model Number:';
$_['entry_image']          = 'Image:';
$_['entry_barcode']        = 'Barcode Digits:';
$_['entry_barcode_encoding'] = 'Barcode Encoding:';

// Error
$_['error_permission']     = 'Warning: You do not have permission to modify products with options';
$_['error_ean']            = 'EAN-13/ISBN barcode must be 12 or 13 digit!'; // New
$_['error_upc']            = 'UPC barcode must be 11 or 12 digit!'; // New
$_['error_barcode_already_exists'] = '* ERROR - This barcode already EXISTS *';
?>

...
class ControllerModuleCatalogInformation extends Controller {
	...
	function __construct(&$locator){
		...
		$this->adminController = $this->template->set_controller('module_catalog_information');
	...
	function index() { 
		...
		if ($this->request->isPost() && $this->request->has('catalog_information_status', 'post') && $this->validate()) {
			...
			if ($this->request->has('update_form', 'post')) {
				$this->response->redirect($this->url->ssl('module_catalog_information'));
			} else {
		...
		$this->session->set('last_module', 'module_catalog_information');
		$this->session->set('last_extension_id', $this->modelInformation->get_extension_id('module_catalog_information'));
	...
	function uninstall() {
		if ($this->user->hasPermission('modify', 'module_catalog_information')) {
			$this->modelInformation->delete_information();
			if ($this->session->get('last_module') == 'module_catalog_information') {

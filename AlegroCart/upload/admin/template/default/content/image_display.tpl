...
function removeModule(row) {
  ...
  <script type="text/javascript"><!--
	...
	function copySlides() {
		var html ='';
		<?php foreach ($image_display_descriptions as $image_display_description) { ?>
			var boxes = document.querySelectorAll("[name^='sliderimage_id[<?php echo $image_display_description['language_id']; ?>]']").length;
			for (i =0; i < boxes; i++) {
				if (document.forms['form'].elements['sliderimage_id[<?php echo $image_display_description['language_id']; ?>]['+[i]+']'] !=undefined){
				html +='<input type="hidden" name="sliderimage_id[<?php echo $image_display_description['language_id']; ?>]' + '[' + [i] + ']" value="' + document.forms['form'].elements['sliderimage_id[<?php echo $image_display_description['language_id']; ?>]['+[i]+']'].value + '">';
				html +='<input type="hidden" name="sort_order[<?php echo $image_display_description['language_id']; ?>]' + '[' + [i] + ']" value="' + document.forms['form'].elements['sort_order[<?php echo $image_display_description['language_id']; ?>]['+[i]+']'].value + '">';
				}
			}
		<?php } ?>

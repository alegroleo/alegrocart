...
$files=array('cache'.D_S, 'image'.D_S, 'image'.D_S.'cache'.D_S, 'image'.D_S.'flash'.D_S, 'image'.D_S.'mask'.D_S, 'image'.D_S.'barcode'.D_S, 'image'.D_S.'watermark'.D_S ,'download'.D_S);
...
$files=array('logs'.D_S, 'logs'.D_S.'error_log'.D_S, 'logs'.D_S.'access_log'.D_S);
foreach ($files as $file) {
	$file=DIR_BASE.$file;
	if (!is_writable($file)) { 
		@chmod($file, 0750);

...
class Model_Admin_Zone extends Model {
	...
	function insert_zone(){
		$sql = "insert into zone set name = '?', zone_status = '?',code = '?', country_id = '?', date_added = now()";
	...
	function get_deleted_log(){
		$result = $this->database->getRow("SELECT CONCAT(firstname, ' ', lastname) AS modifier FROM zone_log zl INNER JOIN user u ON (u.user_id = zl.trigger_modifier_id) WHERE zone_id = '" . (int)$this->request->gethtml('zone_id') . "' AND trigger_action = 'DELETE'");
		return $result;
	}

	function get_modified_log($date_modified){
		$result = $this->database->getRow("SELECT zl.*, CONCAT(firstname, ' ', lastname) AS modifier FROM zone_log zl INNER JOIN user u ON (u.user_id = zl.trigger_modifier_id) WHERE zone_id = '" . (int)$this->request->gethtml('zone_id') . "' AND date_modified =  '" . $date_modified . "'");
		return $result;
	}
	...
	function get_vendorZones(){
		$result = $this->database->getRows("SELECT distinct zone_id FROM vendor_address va LEFT JOIN vendor v ON (v.address_id = va.address_id) WHERE v.status='1'");
		return $result;
	}

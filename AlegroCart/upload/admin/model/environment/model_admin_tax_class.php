...
class Model_Admin_Tax_Class extends Model {
	...
	function update_tax_class(){
		$sql = "update tax_class set title = '?', description = '?' where tax_class_id = '?'";
	...
	function get_deleted_log(){
		$result = $this->database->getRow("SELECT CONCAT(firstname, ' ', lastname) AS modifier FROM tax_class_log tcl INNER JOIN user u ON (u.user_id = tcl.trigger_modifier_id) WHERE tax_class_id = '" . (int)$this->request->gethtml('tax_class_id') . "' AND trigger_action = 'DELETE'");
		return $result;
	}

	function get_modified_log($date_modified){
		$result = $this->database->getRow("SELECT tcl.*, CONCAT(firstname, ' ', lastname) AS modifier FROM tax_class_log tcl INNER JOIN user u ON (u.user_id = tcl.trigger_modifier_id) WHERE tax_class_id = '" . (int)$this->request->gethtml('tax_class_id') . "' AND date_modified =  '" . $date_modified . "'");
		return $result;
	}

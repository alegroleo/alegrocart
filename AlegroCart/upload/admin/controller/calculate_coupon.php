...
class ControllerCalculateCoupon extends Controller {
	...
	function index() { 
		...
		if ($this->request->isPost() && $this->request->has('global_coupon_status', 'post') && $this->validate()) {
			...
			if ($this->request->has('update_form', 'post')) {
				$this->response->redirect($this->url->ssl('calculate_coupon'));
			} else {
		...
		$this->session->set('last_calculate', 'calculate_coupon');
		$this->session->set('last_extension_id', $this->modelCoupon->get_extension_id('calculate_coupon'));
	...
	function uninstall() {
		if ($this->user->hasPermission('modify', 'calculate_coupon')) {
			$this->modelCoupon->delete_coupon();
			if ($this->session->get('last_calculate') == 'calculate_coupon') {

...
$base = htmlentities(HTTPS_BASE ? HTTPS_SERVER : HTTP_SERVER, ENT_QUOTES, 'UTF-8');
...
if ($config->get('config_url_alias')) {
	...
	if ($config->get('manufacturer_status')) {
		...
		//Add Manufacturers
		$sql = "SELECT `alias` FROM `url_alias` WHERE `query` LIKE 'controller=manufacturer&manufacturer_id=%' ";
		$results = $database->getRows($sql);
		foreach($results as $result) {
			$priority = '0.' . rand(6,9);
			echo '<url>' . "\n";
		...
		//Add Products in Manufacturers 
		$sql = "SELECT `alias` FROM `url_alias` WHERE `query` LIKE 'controller=product&manufacturer_id=%' ";
		$results = $database->getRows($sql);
		foreach($results as $result) {
			$priority = '0.' . rand(8,9);
			echo '<url>' . "\n";
	...
	//Add Products in Categories or Subcategories 
	$sql = "SELECT `alias` FROM `url_alias` WHERE `query` LIKE 'controller=product&path=%' ";
	$results = $database->getRows($sql);
	foreach($results as $result) {
		$priority = '0.' . rand(8,9);
		echo '<url>' . "\n";
	...
	$sql = "SELECT `alias` FROM `url_alias` WHERE `query` LIKE 'controller=category&path=%' ";
	$results = $database->getRows($sql);
	foreach($results as $result) {
		$priority = '0.' . rand(6,9);
		echo '<url>' . "\n";

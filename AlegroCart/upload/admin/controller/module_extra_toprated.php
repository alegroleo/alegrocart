...
class ControllerModuleExtraToprated extends Controller {
	...
	function __construct(&$locator){
		...
		$this->adminController = $this->template->set_controller('module_extra_toprated');
	...
	function index() { 
		...
		if (($this->request->isPost()) && ($this->validate())) {
			...
			if ($this->request->has('update_form', 'post')) {
				$this->response->redirect($this->url->ssl('module_extra_toprated'));
			} else {
		...
		$this->session->set('last_module', 'module_extra_toprated');
		$this->session->set('last_extension_id', $this->modelToprated->get_extension_id('module_extra_toprated'));
		...
		if ($this->request->has('catalog_toprated_slider', 'post')) {
			$view->set('catalog_toprated_slider', $this->request->gethtml('catalog_toprated_slider', 'post'));
		} else {
			$view->set('catalog_toprated_slider', @$setting_info['catalog']['toprated_slider']);
		}
		if ($this->request->has('catalog_toprated_slimit', 'post')) {
			$view->set('catalog_toprated_slimit', $this->request->gethtml('catalog_toprated_slimit', 'post'));
		} else {
			$view->set('catalog_toprated_slimit', @$setting_info['catalog']['toprated_slimit']);
		}
		if ($this->request->has('catalog_toprated_scolumns', 'post')) {
			$view->set('catalog_toprated_scolumns', $this->request->gethtml('catalog_toprated_scolumns', 'post'));
		} else {
			$view->set('catalog_toprated_scolumns', @$setting_info['catalog']['toprated_scolumns']);
		}
	...
	function uninstall() {
		if ($this->user->hasPermission('modify', 'module_extra_toprated')) {
			$this->modelToprated->delete_toprated();
			if ($this->session->get('last_module') == 'module_extra_toprated') {

...
class ControllerModuleExtraFeatured extends Controller {
	...
	function index() { 
		...
		if (($this->request->isPost()) && ($this->validate())) {
			...
			if ($this->request->has('update_form', 'post')) {
				$this->response->redirect($this->url->ssl('module_extra_featured'));
			} else {
		...
		$this->session->set('last_module', 'module_extra_featured');
		$this->session->set('last_extension_id', $this->modelFeatured->get_extension_id('module_extra_featured'));
	...
	function uninstall() {
		if ($this->user->hasPermission('modify', 'module_extra_featured')) {
			$this->modelFeatured->delete_featured();
			if ($this->session->get('last_module') == 'module_extra_featured') {

...
if (filesize('../config.php') == 0) { //install is not done...

	$length = function_exists('mb_strlen') ? mb_strlen($_POST['new_admin_name'],'UTF-8'):strlen($_POST['new_admin_name']);
	$pw_length = function_exists('mb_strlen') ? mb_strlen($_POST['password'],'UTF-8') : strlen($_POST['password']);
	$restricted = array('admin','administration');
	$existing = array('cache','library', 'logs','catalog','image','download','install');

	if (empty($_POST['username'])) {
		$ferrors['admin_uname'] = $language->get('error_admin_uname');
	}
	if (empty($_POST['password'])) {
		$ferrors['admin_passw'] = $language->get('error_admin_passw');
	}
	if (!empty($_POST['password']) && ($pw_length < 6 || $pw_length > 20)) {
		$ferrors['admin_passw_lenght'] = $language->get('error_admin_passw_lenght');
	}
	...
	if (!$errors && !$ferrors) {
			...
			if ($handle3 = fopen($file3, 'a+')) {
				$sitemap  = 'Sitemap: '. (HTTPS_BASE ? HTTPS_BASE : HTTP_BASE) . 'sitemap.php'."\n";
	...
	if (!$errors && !$ferrors) {
		...
		//we modify settings to enable SSL if SSL supported, so user has nothing to do
		if (HTTPS_BASE) {
			$database->runQuery("DELETE FROM `setting` WHERE `key` = 'config_ssl'");
			$database->runQuery("INSERT INTO `setting` (`setting_id`, `type`, `group`, `key`, `value`) VALUES (NULL, 'admin', 'config', 'config_ssl', '1')");
			$database->runQuery("INSERT INTO `setting` (`setting_id`, `type`, `group`, `key`, `value`) VALUES (NULL, 'catalog', 'config', 'config_ssl', '1')");
		}

...
<?php if (!($product['max_qty']!=0 &&(($product['max_qty'] - $product['cart_level'] == 0) || ($product['max_qty'] - $product['cart_level'] < $product['multiple'])))) { ?>
	...
	<?php } else {?>
		<input name="<?php echo $this_controller;?>_quantity" id="<?php echo $this_controller.'_quantity_'.$product['product_id']; ?>" size="3" maxlength="5" value="<?php echo $product['cart_level'] ? ($product['multiple'] !=0 ? $product['multiple'] : 1) : $product['min_qty'];?>">
	<?php }?>
<?php }?>

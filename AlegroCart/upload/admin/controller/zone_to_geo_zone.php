...
class ControllerZoneToGeoZone extends Controller {
	...
	protected function insert() {
		...
		if ($this->request->isPost() && $this->request->has('country_id', 'post') && $this->validateForm()) {
			...
			$insert_id = $this->modelZonetoGeo->get_last_id();
			$this->session->set('last_zone_to_geo_zone_id', $insert_id);
	...
	protected function update() {
		...
		if ($this->request->isPost() && $this->request->has('country_id', 'post') && $this->validateForm() && $this->validateModification()) {
			...
			if ($this->request->has('update_form', 'post')) {
				$this->response->redirect($this->url->ssl('zone_to_geo_zone', 'update', $query));
			} else {
				$this->response->redirect($this->url->ssl('zone_to_geo_zone', FALSE, array('geo_zone_id' => $this->request->gethtml('geo_zone_id'))));
			}
		}
	...
	protected function addDeleteAll(){
	...
	private function validateEnableALL(){
	...
	private function getList() {
		...
		foreach ($results as $result) {
			$last = $result['zone_to_geo_zone_id'] == $this->session->get('last_zone_to_geo_zone_id') ? 'last_visited': '';
			$cell = array();
			$cell[] = array(
				'value' => $result['name'],
				'align' => 'left',
	...
	private function getForm() {
		...
		$view->set('message', $this->session->get('message'));
		$this->session->delete('message');
		$view->set('zone_to_geo_zone_id', $this->request->gethtml('zone_to_geo_zone_id'));
		...
		$this->session->set('last_zone_to_geo_zone_id', $this->request->gethtml('zone_to_geo_zone_id'));

		if (($this->request->gethtml('zone_to_geo_zone_id')) && (!$this->request->isPost())) {
			$zone_to_geo_zone_info = $this->modelZonetoGeo->get_zoneToGeoID();
			$this->session->set('zone_to_geo_zone_date_modified', $zone_to_geo_zone_info['date_modified']);
		}
	...
	private function validateModification() {
		if ($zone_to_geo_zone_data = $this->modelZonetoGeo->get_zoneToGeoID()) {
			if ($zone_to_geo_zone_data['date_modified'] != $this->session->get('zone_to_geo_zone_date_modified')) {
				$zone_to_geo_zone_data_log = $this->modelZonetoGeo->get_modified_log($zone_to_geo_zone_data['date_modified']);

				if ($zone_to_geo_zone_data_log['country_id'] != $this->request->gethtml('country_id', 'post')) {
					$this->error['country'] = $this->language->get('error_modified', $this->modelZonetoGeo->get_country_name($zone_to_geo_zone_data_log['country_id']));
				}

				if ($zone_to_geo_zone_data_log['zone_id'] != $this->request->gethtml('zone_id', 'post')) {
					$this->error['zone'] = $this->language->get('error_modified', $zone_to_geo_zone_data_log['zone_id'] == 0 ? $this->language->get('text_all_zones') : $this->modelZonetoGeo->get_zone_name($zone_to_geo_zone_data_log['zone_id']));
				}

				$this->session->set('zone_to_geo_zone_date_modified', $zone_to_geo_zone_data_log['date_modified']);
			}
		} else {
			$zone_to_geo_zone_data_log = $this->modelZonetoGeo->get_deleted_log();
			$this->session->set('message', $this->language->get('error_deleted', $zone_to_geo_zone_data_log['modifier']));
			$this->response->redirect($this->url->ssl('zone_to_geo_zone', FALSE, array('geo_zone_id' => $this->request->gethtml('geo_zone_id'))));
		}
		if (@$this->error){
			$this->error['warning'] = $this->language->get('error_modifier', $zone_to_geo_zone_data_log['modifier']);
		}
		if (!$this->error) {
			return TRUE;
		} else {

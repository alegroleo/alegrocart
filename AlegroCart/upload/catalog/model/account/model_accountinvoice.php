...
class Model_AccountInvoice extends Model{
	...
	function get_order_history($order_id){
		$results = $this->database->getRows("select oh.date_added, os.name as status, oh.comment from order_history oh left join order_status os on oh.order_status_id = os.order_status_id where os.language_id = '" . (int)$this->language->getId() . "' and oh.order_id = '" . (int)$order_id . "' and oh.notify = '1' order by oh.date_added");
		return $results;
	}

...
class ControllerCalculateTotal extends Controller {
	...
	function __construct(&$locator){
		...
		$this->adminController = $this->template->set_controller('calculate_total');
	...
	function index() { 
		...
		if ($this->request->isPost() && $this->request->has('global_total_status', 'post') && $this->validate()) {
			...
			if ($this->request->has('update_form', 'post')) {
				$this->response->redirect($this->url->ssl('calculate_total'));
			} else {
		...
		$this->session->set('last_calculate', 'calculate_total');
		$this->session->set('last_extension_id', $this->modelTotal->get_extension_id('calculate_total'));
	...
	function uninstall() {
		if ($this->user->hasPermission('modify', 'calculate_total')) {
			$this->modelTotal->delete_total();
			if ($this->session->get('last_calculate') == 'calculate_total') {

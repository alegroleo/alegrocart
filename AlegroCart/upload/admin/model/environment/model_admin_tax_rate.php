...
class Model_Admin_Tax_Rate extends Model {
	...
	function update_taxrate(){
		$sql = "update tax_rate set geo_zone_id = '?', tax_class_id = '?', priority = '?', rate = '?', description = '?' where tax_rate_id = '?'";
	...
	function get_deleted_log(){
		$result = $this->database->getRow("SELECT CONCAT(firstname, ' ', lastname) AS modifier FROM tax_rate_log trl INNER JOIN user u ON (u.user_id = trl.trigger_modifier_id) WHERE tax_rate_id = '" . (int)$this->request->gethtml('tax_rate_id') . "' AND trigger_action = 'DELETE'");
		return $result;
	}

	function get_modified_log($date_modified){
		$result = $this->database->getRow("SELECT trl.*, CONCAT(firstname, ' ', lastname) AS modifier FROM tax_rate_log trl INNER JOIN user u ON (u.user_id = trl.trigger_modifier_id) WHERE tax_rate_id = '" . (int)$this->request->gethtml('tax_rate_id') . "' AND date_modified =  '" . $date_modified . "'");
		return $result;
	}

	function get_geozone_name($geo_zone_id){ 
		$result = $this->database->getRow("SELECT name FROM geo_zone WHERE geo_zone_id = '" . (int)$geo_zone_id . "'");
		return $result['name'];
	}

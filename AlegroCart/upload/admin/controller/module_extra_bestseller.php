...
class ControllerModuleExtraBestseller extends Controller {
	...
	function __construct(&$locator){
		...
		$this->modelBestseller = $model->get('model_admin_bestseller');
		$this->head_def		=& $locator->get('HeaderDefinition');
		$this->adminController = $this->template->set_controller('module_extra_bestseller');

		$this->language->load('controller/module_extra_bestseller.php');
	}
	...
	function index() { 
		...
		if (($this->request->isPost()) && ($this->validate())) {
			...
			if ($this->request->has('update_form', 'post')) {
				$this->response->redirect($this->url->ssl('module_extra_bestseller'));
			} else {
		...
		$this->session->set('last_module', 'module_extra_bestseller');
		$this->session->set('last_extension_id', $this->modelBestseller->get_extension_id('module_extra_bestseller'));
		...
		if (!$this->request->isPost()) {
			$results = $this->modelBestseller->get_bestseller();
			foreach ($results as $result) {
				$setting_info[$result['type']][$result['key']] = $result['value'];
			}
		}

		if ($this->request->has('catalog_bestseller_status', 'post')) {
		...
		if ($this->request->has('catalog_bestseller_total', 'post')) {
			...
			$view->set('catalog_bestseller_total', @$setting_info['catalog']['bestseller_total']);
		}
		if ($this->request->has('catalog_bestseller_image_width', 'post')) {
			$view->set('catalog_bestseller_image_width', $this->request->gethtml('catalog_bestseller_image_width', 'post'));
		} else {
			$view->set('catalog_bestseller_image_width', @$setting_info['catalog']['bestseller_image_width']);
		}
		if ($this->request->has('catalog_bestseller_image_height', 'post')) {
			$view->set('catalog_bestseller_image_height', $this->request->gethtml('catalog_bestseller_image_height', 'post'));
		} else {
			$view->set('catalog_bestseller_image_height', @$setting_info['catalog']['bestseller_image_height']);
		}
		if ($this->request->has('catalog_bestseller_addtocart', 'post')) {
			$view->set('catalog_bestseller_addtocart', $this->request->gethtml('catalog_bestseller_addtocart', 'post'));
		} else {
			$view->set('catalog_bestseller_addtocart', @$setting_info['catalog']['bestseller_addtocart']);
		}
		if ($this->request->has('catalog_bestseller_columns', 'post')) {
			$view->set('catalog_bestseller_columns', $this->request->gethtml('catalog_bestseller_columns', 'post'));
		} else {
			$view->set('catalog_bestseller_columns', @$setting_info['catalog']['bestseller_columns']);
		}
		if ($this->request->has('catalog_bestseller_image_display', 'post')) {
		...
		if ($this->request->has('catalog_bestseller_lines_single', 'post')) {
			...
			$view->set('catalog_bestseller_lines_single', @$setting_info['catalog']['bestseller_lines_single']);
		}
		if ($this->request->has('catalog_bestseller_lines_multi', 'post')) {
			$view->set('catalog_bestseller_lines_multi', $this->request->gethtml('catalog_bestseller_lines_multi', 'post'));
		} else {
			$view->set('catalog_bestseller_lines_multi', @$setting_info['catalog']['bestseller_lines_multi']);
		}
		if ($this->request->has('catalog_bestseller_lines_char', 'post')) {
			$view->set('catalog_bestseller_lines_char', $this->request->gethtml('catalog_bestseller_lines_char', 'post'));
		} else {
	...
	function validate() {
		...
		if (!$this->user->hasPermission('modify', 'module_extra_bestseller')) {
	...
	function install() {
		if ($this->user->hasPermission('modify', 'module_extra_bestseller')) {
			$this->modelBestseller->delete_bestseller();
			$this->modelBestseller->install_bestseller();
			$this->session->set('message', $this->language->get('text_message'));
		} else {
	...
	function uninstall() {
		if ($this->user->hasPermission('modify', 'module_extra_bestseller')) {
			$this->modelBestseller->delete_bestseller();
			if ($this->session->get('last_module') == 'module_extra_bestseller') {

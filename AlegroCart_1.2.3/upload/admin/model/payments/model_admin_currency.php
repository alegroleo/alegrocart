...
class Model_Admin_Currency extends Model {
	...
	function insert_currency(){
		...
		$this->database->query($this->database->parse($sql, $this->request->gethtml('title', 'post'), $this->request->gethtml('code', 'post'), $this->request->gethtml('symbol_left', 'post'), $this->request->gethtml('symbol_right', 'post'), $this->request->gethtml('decimal_place', 'post'), number_format($this->request->gethtml('value', 'post'), 8)));
	}
	function update_currency(){
		$sql = "update currency set title = '?', code = '?', symbol_left = '?', symbol_right = '?', decimal_place = '?', value = '?', date_modified = now() where currency_id = '?'";
		$this->database->query($this->database->parse($sql, $this->request->gethtml('title', 'post'), $this->request->gethtml('code', 'post'), $this->request->gethtml('symbol_left', 'post'), $this->request->gethtml('symbol_right', 'post'), $this->request->gethtml('decimal_place', 'post'), number_format($this->request->gethtml('value', 'post'), 8), $this->request->gethtml('currency_id')));
	}
	function delete_currency(){
		$this->database->query("delete from currency where currency_id = '" . (int)$this->request->gethtml('currency_id') . "'");
	}
	function get_codes(){
		$results = $this->database->getRows("select code from currency");
		return $results;
	}
	...
	function get_page(){
		...
		$sort = array('title', 'code', 'value',	'date_modified');

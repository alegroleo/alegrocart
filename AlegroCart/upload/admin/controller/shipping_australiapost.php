...
class ControllerShippingAustraliaPost extends Controller{
	...
	function __construct(&$locator){
		...
		$this->adminController = $this->template->set_controller('shipping_australiapost');
	...
	function index() {  
		...
		if ($this->request->isPost() && $this->request->has('global_australiapost_status', 'post') && $this->validate()) {
			...
			if ($this->request->has('update_form', 'post')) {
				$this->response->redirect($this->url->ssl('shipping_australiapost'));
			} else {
		...
		$this->session->set('last_shipping', 'shipping_australiapost');
		$this->session->set('last_extension_id', $this->modelAUPost->get_extension_id('shipping_australiapost'));
	...
	function uninstall() {
		if ($this->user->hasPermission('modify', 'shipping_australiapost')) {
			$this->modelAUPost->delete_AUP();
			if ($this->session->get('last_shipping') == 'shipping_australiapost') {

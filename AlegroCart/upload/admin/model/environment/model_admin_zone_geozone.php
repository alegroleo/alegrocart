...
class Model_Admin_Zone_GeoZone extends Model {
	...
	function get_deleted_log(){
		$result = $this->database->getRow("SELECT CONCAT(firstname, ' ', lastname) AS modifier FROM zone_to_geo_zone_log z2gzl INNER JOIN user u ON (u.user_id = z2gzl.trigger_modifier_id) WHERE zone_to_geo_zone_id = '" . (int)$this->request->gethtml('zone_to_geo_zone_id') . "' AND trigger_action = 'DELETE'");
		return $result;
	}

	function get_modified_log($date_modified){
		$result = $this->database->getRow("SELECT z2gzl.*, CONCAT(firstname, ' ', lastname) AS modifier FROM zone_to_geo_zone_log z2gzl INNER JOIN user u ON (u.user_id = z2gzl.trigger_modifier_id) WHERE zone_to_geo_zone_id = '" . (int)$this->request->gethtml('zone_to_geo_zone_id') . "' AND date_modified =  '" . $date_modified . "'");
		return $result;
	}
	...
	function insertCountry($geo_zone_id, $country_id){
		$sql = "insert into zone_to_geo_zone set country_id = '?', zone_id = '?', geo_zone_id = '?', date_added = now()";
		$this->database->query($this->database->parse($sql, $country_id, '0', $this->request->gethtml('geo_zone_id')));
	}

...
final class Barcode {
	...
	public function check($code, $encoding){

		$even = true; 
		$esum = 0; 
		$osum = 0; 

		if ($this->get_length($code) < 11) {
			return;
		}
	...
	public function get_length ($code){
	...
	private function href($filename) {
		if (@$_SERVER['HTTPS'] != 'on') {
	...
	private function barcode_encode($code, $encoding){
		...
		for ($i=1;$i<13;$i++) {
			$str=$digits[$code[$i]];
			if ($encoding == 'ean') {
				if ($i<7 && $mirror[$code[0]][$i-1]==1) {
					$line.=strrev($str); 
				} else { 
					$line.=$str;
				}
			} else {
				if ($i<7 && $i>1 && $mirror[$code[0]][$i-1]==1) {
					$line.=strrev($str); 
				} else {
					$line.=$str;
				}
	...
	private function barcode_outimage($text, $bars, $code, $encoding, $total_y = 0, $space = '') {

		/* set defaults */
		if ($this->scale<1) $this->scale=2;
		$total_y=(int)($total_y);
		if ($total_y<1) $total_y=(int)$this->scale * 60;
		if (!$space) $space=array('top'=>2*$this->scale,'bottom'=>2*$this->scale,'left'=>2*$this->scale,'right'=>2*$this->scale);
		...
		foreach ($chars as $n => $v) {
			if (trim($v)){
				...
				@imagettftext($im, $fontsize, 0, $space['left']+($this->scale*$inf[0])+2, $fontheight, $col_text, DIR_FONTS.'FreeSansBold.ttf', $inf[2]);
			}
		}
